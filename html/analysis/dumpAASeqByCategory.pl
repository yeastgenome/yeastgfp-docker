...
#    next;
    open(OUTFILE, ">group".$subcellID.".fasta");
    
    $totalCount = 0;

    while( my ($key, $value) = each(%orfHash) ) {
	$statementGetSeq = "select sequence from orfproteinsequence where orfid=".$key;
	...
	open(RANDFILE, ">rand_group".$subcellID."_try".$randNum);
	...
	for($i=0; $i<$totalCount; $i++) {
	    ...
	    $statementGetSeq = "select sequence from orfproteinsequence where orfid=".$randOrfID;
	    $sthGetSeq = $dbh->prepare($statementGetSeq);
	    $sthGetSeq->execute();
	    $hash_ref = $sthGetSeq->fetchrow_hashref;
